...
import { searchUsersService } from "../services/user/search.service";
import {
  getAllUsersService,
...
export const getAllUsers: RequestHandler = async (req, res) => {
  try {
    ...
    const page = parseInt(pagQuery as string);
...
export const uploadUserAvatar: RequestHandler = async (req, res) => {
  try {
    ...
    const updatedUser = await uploadUserAvatarService(email, imagePath);
...
export const updateInfoByEmail: RequestHandler = async (req, res) => {
  ...
  try {
    const updatedInfo = await updateUserInfoService(email, {
      gender,
      address,
      birthday,
    });
